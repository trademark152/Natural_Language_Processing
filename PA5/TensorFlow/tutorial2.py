...
# called twice on training doc and test doc
# x is an array, each entry is the dict of word
def MakeDesignMatrix(x):
    global TERM_INDEX
    if TERM_INDEX is None:
        min_count, max_count = numpy.percentile(VOCABULARY.values(), [70.0, 99.5])
        TERM_INDEX = {}
...
def print_f1_measures(probs, y_test):
    # accuracy = numpy.mean(((probs>0) == (y_test>0)).sum(axis=1) == 2)
    ...
    for attribute_name, score in [('truthful', truthful),
        ...
        all_f1.append(f1)
        print
        '{0:9} {1:.2f} {2:.2f} {3:.2f}'.format(attribute_name, precision, recall, f1)
    print
    'Mean F1: {0:.4f}'.format(float(sum(all_f1)) / len(all_f1))
...
def main(argv):
    ...
    def step(lr=0.01):
        ...
        for si in xrange(0, x_train.shape[0], batch_size):
